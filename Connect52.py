...
def self_play(tree,c,t): # c = level of exploration
    ...
    while abs(Game.result(CurrentState)) == 2:
        ...
        CurrentNode = Node(CurrentState,NextMove,CurrentNode) # always genernate a new node
        steps = steps + 1
...
class Loss(nn.Module):
  ...
  def forward(self,output,target):
    ...
    loss = torch.sum((firstTerm - secondTerm))/steps + l2_norm
    return loss
