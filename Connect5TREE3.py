...
class Tree():
    ...
    def __init__(self,visited,f): # f: CNN
    ...
    def expansion(self,node,root):
        if self.VisitedList == []:
          self.VisitedList.append(node)
        State = node.state
        result = Game.result(State)
        if abs(result) == 1:
            node.children = [] # end state
            Tree.backup(self,node,float(self.policy(State.board)[1]),root)
        ...
        for position in LegalMoves:
            ...
            Tree.backup(self,NewNode,float(self.policy(State.board)[1]),root)
    
    def backup(self,node,num,root):
        node.N += 1
        node.W = node.W + num
        node.Q = node.W / node.N
        parent = node.parent
        if not (parent == root):
          self.backup(parent,1-num,root)
...
def MCTS(node,tree,c,t,root):
    parent = node.parent
    if type(parent) != list:
      vis = parent.children
      for item in vis:
          if item == node:
              LeafNode = Tree.selection(tree,item,c)
              if Node.end(LeafNode):
                  return Tree.pi(tree,item,t)
              else:
                  Tree.expansion(tree,LeafNode,root)
                  return Tree.pi(tree,item,t)
    else:
      vis = Tree.visited(tree,node)
      if vis[0]:
        ActualNode = vis[1]
        LeafNode = Tree.selection(tree,ActualNode,c)
        Tree.expansion(tree,LeafNode,root)
        return Tree.pi(tree,node,t)
    Tree.expansion(tree,node,root)
    return Tree.pi(tree,node,t)
...
def self_play(tree,c,t): # c = level of exploration
    ...
    while abs(Game.result(CurrentState)) == 2:
        CurrentPi = MCTS(CurrentNode,tree,c,t,CurrentNode) # _,_,_,_,root
...
# defined loss function and optimizer
    ...
    while count < 100000:
        if count <= 30:
            ...
            with open("Tree", "wb") as fp:   #Pickling
                pickle.dump(tree.VisitedList,fp)
            count += 1
        else:
            c = 0.5
            t = 0.1
